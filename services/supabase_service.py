...
def create_payment_record(
    ...
    print(f"[supabase] create_payment_record: {telegram_id} | plan={plan} | ref={reference}")
    result = (
    ...
    print(f"[supabase] Payment record created.")
    return result.data[0] if result.data else {}


def confirm_payment(reference: str, expires_at: str) -> dict:
    print(f"[supabase] confirm_payment: ref={reference}")
    result = (
        supabase.table("payments")
        .update({
            "status":      "success",
    ...
    print(f"[supabase] Payment confirmed in DB.")
    return result.data[0] if result.data else {}


def get_payment_by_reference(reference: str) -> dict | None:
    print(f"[supabase] get_payment_by_reference: {reference}")
    result = (
        supabase.table("payments")
        .select("*")
        .eq("paystack_reference", reference)
...
def get_latest_payment(telegram_id: int) -> dict | None:
    print(f"[supabase] get_latest_payment: {telegram_id}")
    result = (
        supabase.table("payments")
        .select("*")
        .eq("telegram_id", telegram_id)
